...
def get_user_quiz(userid, quizid):
    '''Returns a quiz using userid and quizid.'''
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def set_user_quiz(userid, quizid, sent, answer, result):
    '''Set in DB a new quiz using userid.'''
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
...
def main():
    ...
    return render_template('index.html', username=auth.username(), challenges=challenges, quiz=quiz[0], e=(sent > quiz[0][2]), answers=answers, p=p, msg=msg, expi=convete_data(quiz[0][2]))
